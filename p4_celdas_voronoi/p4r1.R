...
#Reto1: distribuir las semillas de otras formas
...
for (op in 1:4) {
  ...
  for (semilla in 1:k) {
    while (TRUE) { # hasta que hallamos una posicion vacia para la semilla
      if (op == 1) { #mitad superior
        fila <- sample(1:(n/2), 1) 
        columna <- sample(1:n, 1)
        dist = "superior"
      }
      else if (op == 2) { #derecha
        fila <- sample(1:n, 1)
        columna <- sample((n/2):n, 1) 
        dist = "derecha"
      }
      else if (op == 3) { #frontera
        ini <- inicio()
        fila <- ini[1]
        columna <- ini[2]
        dist = "frontera"
        ...
        fila <- sample(((n/2)-10):((n/2)+10), 1) 
        columna <- sample(((n/2)-10):((n/2)+10), 1)
        dist = "centro"
      }
  ...
  png(paste("p4s_", dist, ".png", sep=""))
      par(mar = c(0,0,0,0))
      image(rotate(zona), col=rainbow(k+1), xaxt='n', yaxt='n')
      graphics.off() 
      
      png(paste("p4c_", dist, ".png", sep=""))
      par(mar = c(0,0,0,0))
      ...
      resultados <- rbind(resultados, c(op,largos))
}

resultados <- resultados[,-1]
data <- t(resultados)

distribucion <- c("S", "D", "F", "C")

png("p4dist.png")
colnames(data) <- (distribucion) 
boxplot(data, xlab="Distribuci\u{F3}n ", ylab="Largos")
graphics.off()
